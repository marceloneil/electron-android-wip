...
JNIEXPORT void JNICALL Java_org_kivy_android_PythonService_nativeStart(
    ...
    jstring j_arg, jstring j_android_ext_dir, jstring j_android_data_dir) {
  jboolean iscopy;
  ...
  const char *android_ext_dir =
      (*env)->GetStringUTFChars(env, j_android_ext_dir, &iscopy);
  const char *android_data_dir =
      (*env)->GetStringUTFChars(env, j_android_data_dir, &iscopy);
  ...
  setenv("ANDROID_EXT_DIR", android_ext_dir, 1);
  setenv("ANDROID_DATA_DIR", android_data_dir, 1);
  setenv("ANDROID_NATIVE_UI", "1", 1);
